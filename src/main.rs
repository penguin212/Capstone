...
pub fn main() {
    ...
    let goal_state: State = state::State {
        x: 200,
        y: 200,
        velocity: 0,
        theta: 0,
    ...
    let path: Vec::<state::State> = calc_paths_no_vel::calc_path_no_vel(start_state, goal_state);
    ...
    'running: loop {
        ...
        let mut texture = texture_creator.create_texture_target(None, 20, 150).unwrap();
        ...
        //     texture_canvas.set_draw_color(Color::RGB(255, 0, 0)); // Set the color to red
        //     texture_canvas.fill_rect(sdl2::rect::Rect::new(0, 0, 30, 150)).expect("Could not draw rectangle");
        // }).unwrap();

        // Now you can draw the texture with rotation
        // let center = None; // Center of rotation
        // canvas.copy_ex(&texture, None, Some(sdl2::rect::Rect::new(x, y, 50, 20)), theta as f64, center, false, false).unwrap();
        canvas.set_draw_color(Color::RGB(200, 200, 200));
        canvas.present();
